...
def computeybar_grader(xTe, OFFSET):
    n, d = xTe.shape
    ybar = np.zeros(n)

    # Feel free to use the following function to compute p(x|y), or not
    # normal distribution is default mu = 0, sigma = 1.
    def normpdf(x, mu, sigma): return np.exp(-0.5 *
                                             np.power((x - mu) / sigma, 2)) / (np.sqrt(2 * np.pi) * sigma)
...
def computehbar_grader(xTe, depth, Nsmall, NMODELS, OFFSET):
    n = xTe.shape[0]
    hbar = np.zeros(n)
